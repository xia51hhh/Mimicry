...
fn find_python() -> Result<String, AppError> {
    ...
    Err(AppError::Sidecar(
        "Python not found. Please install Python 3.10+".into(),
    ))
}
...
pub async fn browser_launch(
    ...
    tracing::info!(
        "browser_launch called: session_id={}, profile_id={:?}",
        sid,
        profile_id
    );
    ...
    if let Some(pid) = profile_id {
        ...
        if let Some(profile) = crate::db::profiles::get(&db, &pid)? {
            tracing::info!(
                "Profile loaded: id={}, user_data_dir={:?}",
                pid,
...
pub async fn browser_install(
    ...
    tracing::info!(
        "Installing: python={}, venv={:?}, sidecar={:?}",
        python,
        venv_dir,
        sidecar_dir
    );
...
pub async fn browser_navigate(
    ...
    sidecar_call(
        sidecar,
        "browser.navigate",
        Some(serde_json::json!({"url": url, "session_id": sid})),
    )
    .await
}
...
#[tauri::command]
pub async fn workflow_stop_execution(
    sidecar: State<'_, Mutex<Sidecar>>,
    session_id: Option<String>,
) -> Result<serde_json::Value, AppError> {
    let sid = session_id.unwrap_or_else(|| "default".into());
...
#[tauri::command]
pub async fn workflow_execution_status(
    sidecar: State<'_, Mutex<Sidecar>>,
    session_id: Option<String>,
) -> Result<serde_json::Value, AppError> {
    let sid = session_id.unwrap_or_else(|| "default".into());
...
#[tauri::command]
pub async fn camoufox_check_update(
    sidecar: State<'_, Mutex<Sidecar>>,
) -> Result<serde_json::Value, AppError> {
