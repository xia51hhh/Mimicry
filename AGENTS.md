...
When multiple agents (separate terminals / Claude sessions) work concurrently, each owns one Trellis task in its own worktree under `.trellis/worktrees/<task-slug>/` on branch `feat/<task-slug>`, and PRs directly to `main`. Peer model — no parent/child.

- **Hard contract**: `.trellis/spec/cross-layer/parallel-development.md` (branch naming, hot-file self-declaration, merge-conflict resolution by reading the peer task's PRD, Conventional Commits format with task-slug scope and Chinese description).
- **When to parallelize**: `.trellis/spec/guides/parallel-task-thinking-guide.md` (5-step entry flow + decision tree).
- **Observability**: `SessionStart` and `UserPromptSubmit` hooks auto-inject a `<peer-worktrees>` block, so every agent always sees what other agents are doing without running commands. Manual: `python3 .trellis/scripts/task.py worktree list` and `task.py list --hotfile <path>`.
