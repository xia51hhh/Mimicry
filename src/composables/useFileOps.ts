...
const IMPORT_FILTERS = [{ name: 'Mimicry Workflow', extensions: ['mimicry.json', 'json'] }];
...
export function useFileOps() {
  ...
  async function readFile(path: string) {
    const data = await invoke<{ name: string; nodes: unknown[]; edges: unknown[] }>('file_read', {
      path,
    });
  ...
  async function readFileTransform(path: string) {
    const data = await invoke<{ name?: string; nodes: unknown[]; edges: unknown[] }>(
      'file_import',
      { path },
    );
