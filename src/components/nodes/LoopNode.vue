<script setup lang="ts">
  ...
  const tooltipText = computed(() => {
    ...
    parts.push(`${t('tooltip.loopType')}: ${props.data.loopType}`);
    if (props.data.count != null) parts.push(`${t('tooltip.count')}: ${props.data.count}`);
...
<style scoped>
  ...
  .body-out .branch-label {
    color: #9c27b0;
  }
  .done-out .branch-label {
  ...
  @keyframes pulse-paused {
    0%, 100% { box-shadow: 0 0 8px rgba(255, 167, 38, 0.4); }
    50% { box-shadow: 0 0 16px rgba(255, 167, 38, 0.7); }
  }
