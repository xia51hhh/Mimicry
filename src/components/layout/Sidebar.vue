...
<template>
  <aside v-show="visible" class="sidebar" :style="{ width: sidebarWidth + 'px' }">
        ...
        <div class="sidebar-body">
          <div v-for="cat in blockCategories" :key="cat.key" class="category">
            <button class="category-header" @click="toggleCategory(cat.key)">
              <span class="category-arrow" :class="{ collapsed: collapsedCategories.has(cat.key) }"
                >▼</span
              >
              <span class="category-label">{{ t(`blockCategories.${cat.key}`) }}</span>
            </button>
            <div v-show="!collapsedCategories.has(cat.key)" class="category-items">
              <div
