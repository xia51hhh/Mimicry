...
> **状态**: Planned | **最后更新**: 2026-05-02

> 现实边界（5/2）：当前代码已有 `kind=group` 节点（视觉分组）、`ExecuteWorkflow` action（嵌套子工作流执行）和基础画布结构；本文描述的 Package 库、实例引用、输入输出端口绑定、展开/折叠编辑和运行时执行语义尚未完整实现。组合等同的最小可用方案是用 `ExecuteWorkflow` 调用单独的工作流 JSON 文件。
...
目标设计中，Package 是多个 Block 的视觉封装单元，在画布上呈现为单个节点，但内部包含完整的子 Block 图。
