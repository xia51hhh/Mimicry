...
目标设计中，Block 之间通过连线自动传递数据。当前实现会保留 `edges`，但 Python executor 主要按 `nodes` 数组和 `children` / `elseChildren` 顺序执行，尚未用边作为完整调度图：
...
目标设计中，Mimicry 使用 `{{}}` Mustache 风格的表达式语法，在 Block 的 `data` 字段中引用动态数据。当前 executor 已具备部分条件表达式解析能力，但完整 `$prev` / `$loop` / `$browser` / `$env` 上下文注入仍需继续实现。
...
参考 [方案对比](../research/solution-comparison.md) Section E.3（注：最终方案采用了简化的单值传递模型而非 E.2 推荐的 Items 流）。当前 canonical node 方向以 `kind + action + data + settings` 为准，以下完整结构是目标 schema，部分字段尚未由运行时完整消费：
