...
- Battery API 伪装
- **Stealth JS 注入**：`add_init_script` 在所有页面加载前注入 webdriver 描述符修复 + iframe 一致性补丁（`controller.py._STEALTH_JS`）
...
### Camoufox Issue #555 — Akamai Bot Manager 检测 v135 补丁（已修复）
...
- **状态**：✅ **已修复** — 升级到 CoryKing fork v142+ 后通过

### Camoufox Issue #536 — PerimeterX 检测（已修复）
- **现象**：PerimeterX（Outlook 注册等）标记 Camoufox，同 IP 原版 Firefox 不受影响
- **状态**：✅ **已修复** — 与 #555 同源，v142+ 已修复
...
> 测试环境：Linux x86_64，Camoufox v0.4.12 (CoryKing fork, Firefox 142.0.1-fork.26)
...
| Bing | ✅ PASS | 逐字符输入 "hello world" | v142 修复了 Akamai #555 检测，搜索正常 |
...
| [BrowserScan](https://www.browserscan.net/) | ✅ PASS | 指纹扫描完成 |
| [CreepJS](https://abrahamjuliot.github.io/creepjs/) | ✅ PASS | 指纹分析正常运行 |
| [Incolumitas](https://bot.incolumitas.com/) | ⚠️ PARTIAL | 28/29 测试通过，仅 `webDriverAdvanced: FAIL`（C++ 引擎层限制，无法 JS 层修复） |
| [SannySoft](https://bot.sannysoft.com/) | ✅ PASS | 0 项红色标记 |
| [BrowserLeaks WebRTC](https://browserleaks.com/webrtc) | ✅ PASS | WebRTC 完全阻断 (`RTCPeerConnection is not defined`) |
| [PixelScan](https://pixelscan.net/) | ✅ PASS | 指纹一致性通过 |

### V142 升级改进（2026-04-29）

| 检测项 | v135 结果 | v142 结果 | 说明 |
|--------|----------|----------|------|
| Akamai Bot Manager | ❌ FAIL | ✅ PASS | C++ 补丁被重写，不再触发行为分析 |
| PerimeterX | ❌ FAIL | ✅ PASS | 同源修复 |
| Bing 搜索 | ❌ FAIL | ✅ PASS | Akamai 层不再拦截 |
| Incolumitas webDriverAdvanced | ❌ FAIL | ❌ FAIL | 上游 #516，C++ 引擎层限制 |

### CLI 交互测试结果（2026-04-29，v142）
...
| 7 | Bing | ✅ PASS | type "hello world" → 搜索结果正常返回（v142 修复） |

### Bing 修复说明（v142）

**原失败原因**：Camoufox v135 的 C++ 补丁（`no-css-animations.patch`、`canvas-spoofing.patch`、`font-list-spoofing.patch`）在 JS 层可被 Akamai 行为分析检测。

**修复方式**：升级到 CoryKing fork v142（Firefox 142.0.1-fork.26），该版本重写了被检测的补丁。

**验证结果**：Bing 搜索、Akamai 保护站点均已正常通过。
